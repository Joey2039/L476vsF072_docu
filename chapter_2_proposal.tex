...
\begin{figure}[htbp]
  ...
  \caption[AES-128-GCM mode of operation]{AES-128-GCM mode of operation. Adapted from a
    figure by D.\ Maimut in ``TikZ for Cryptographers'' \cite{jean2016tikz} (CC0), following
    the GCM specification \cite{NISTSP80038D}.}
  \label{fig:scheme_gcm}
\end{figure}
...
ASCON uses a sponge-based construction (duplex variant)
...
\begin{figure}[htbp]
  ...
  \caption[Ascon-AEAD128 mode of operation]{Ascon-AEAD128 mode of operation, from the Ascon
    specification \cite{ASCONSpec}. In the diagram $a = 12$ for initialization and finalization,
    and $b = 8$ for data processing.}
  \label{fig:scheme_ascon}
\end{figure}
...
Figures~\ref{fig:scheme_gcm} and~\ref{fig:scheme_ascon} show the two constructions schematically.
